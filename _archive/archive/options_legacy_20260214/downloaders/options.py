...
def build_selected_instruments(
    ...
    Criterion: (creation_timestamp, expiration_timestamp) overlaps the window, i.e.
    creation_timestamp <= end_ms AND expiration_timestamp >= start_ms.
    Strikes within ±ATM_PERCENT_RANGE, cap MAX_STRIKES_PER_SIDE per side.
    """
    if not instruments:
        return []
    ...
    # Creation time: was the option listed by end of window?
    ...
    # Was trading during [start_ms, end_ms] <=> listed before window end and not expired before window start
    was_live_in_window = (df["creation_timestamp"] <= end_ms) & (df["expiration_timestamp"] >= start_ms)
    df = df[was_live_in_window].copy()
    if df.empty:
        logger.warning(
            "No Deribit options had trading life overlapping [%s, %s]. "
            "Need creation_timestamp <= end and expiration_timestamp >= start. "
            "Deribit 'expired=true' may only return recently expired options.",
            ms_to_utc(start_ms).date(),
            ms_to_utc(end_ms).date(),
        )
        return []
...
def persist_selected_instruments(selected: List[Dict[str, Any]]) -> None:
    """Write selected instrument list to disk for reproducibility."""
...
def fetch_candles_chunk(
    ...
    """Get one chunk of 5m candles from Deribit (TradingView)."""
...
def candles_result_to_rows(result: Dict[str, Any], instrument_name: str) -> List[Dict[str, Any]]:
    """Convert get_tradingview_chart_data result to list of row dicts (open_time, open, high, low, close, volume)."""
...
def download_options_klines_for_instrument(
    ...
    """
    Download 5m candles for one option; write partitioned parquet. Returns row count.
    Schema: instrument_name, expiry, strike, call_put, open_time, open, high, low, close, volume, source=deribit.
    """
    ...
    # Validate: drop rows with null/invalid prices
    ...
    # Partition by instrument (filesystem-safe: replace : for path)
...
def download_all_options(
    ...
    """
    Download Deribit instruments, build selected list (or load persisted), download 5m klines.
    Returns (exchange_info_like dict for compatibility, total_option_klines_rows).
    """
    ...
    instruments_per_expiry: Dict[int, int] = {}
    for inv in selected:
        exp = inv.get("expiration_timestamp")
        if exp is not None:
            instruments_per_expiry[exp] = instruments_per_expiry.get(exp, 0) + 1
        bucket = _expiry_bucket(inv.get("expiration_timestamp") or 0)
        buckets[bucket] = buckets.get(bucket, 0) + 1

    logger.info("Deribit options: %s expiries in range, %s instruments selected", len(instruments_per_expiry), len(selected))
    for b, c in sorted(buckets.items()):
        logger.info("  %s: %s instruments", b, c)
    ...
    # Compatibility: return a dict that has "optionSymbols" count for backfill script
    exchange_info_compat = {"optionSymbols": instruments, "deribit_instruments": instruments}
    return exchange_info_compat, total


def get_options_summary_stats() -> Dict[str, Any]:
    """Return counts for console summary: instruments per bucket, total instruments, rows per month (year-month)."""
    selected = load_selected_instruments()
    if not selected:
        ...
        for root, _dirs, files in os.walk(base):
            for f in files:
                if not f.endswith(".parquet"):
                    continue
                try:
                    # Partition layout: .../year=YYYY/month=MM/klines_5m.parquet
