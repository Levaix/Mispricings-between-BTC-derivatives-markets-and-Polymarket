"""Build options IV smiles: 3h window + 90min staleness + min 3 strikes + max gap 10k.

Methodology variant 3: outputs go to this folder only.
...
- Strike gap rule: interpolate only if local bracket gap <= 10k; else iv_hat = NaN, out_of_range = True.
"""
...
ROLLING_WINDOW_MINUTES = 180
STALENESS_MINUTES = 90
MIN_POINTS = 3
MAX_STRIKE_GAP = 10000.0
DAILY_EXPIRY_CUTOFF_HOUR_UTC = 8


def chosen_daily_expiry(event_date_str: str, bucket_ts: pd.Timestamp) -> str:
    """For event date Z and timestamp t: option expiry = Z if t <= Z 08:00 UTC else Z+1."""
    t = bucket_ts
    if t.tzinfo is None:
        t = t.tz_localize("UTC")
...
def load_options(path: Path) -> pd.DataFrame:
    ...
    required = {
        "instrument_name", BUCKET_COL, "iv_vwap", "contracts_sum", "amount_sum",
        "index_price_last", "timestamp_last",
    }
...
def parse_instrument(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, int]]:
    ...
    parse_stats["bad_expiry_rows"] = int(bad_date.sum())
...
def normalize_iv(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, float]]:
    ...
    if np.isnan(med):
        df["iv_norm"] = iv
        decision, factor = "unknown_assume_decimal", 1.0
    elif med > 3.0:
        df["iv_norm"] = iv / 100.0
        decision, factor = "percent_to_decimal", 0.01
    else:
        df["iv_norm"] = iv
        decision, factor = "already_decimal", 1.0
    stats = {"iv_median_raw": med, "iv_scale_decision": decision, "iv_scale_factor": factor}
    logger.info("IV normalization: %s (median raw=%.4f)", decision, med)
    return df, stats


def load_events(path: Path) -> pd.DataFrame:
    if not path.exists():
        raise FileNotFoundError(f"Events file not found at {path}")
    ev = pd.read_parquet(path) if path.suffix.lower() == ".parquet" else pd.read_csv(path)
    if "expiry_date" not in ev.columns:
        raise ValueError("Events file must contain 'expiry_date'.")
    strike_col = "strike_K" if "strike_K" in ev.columns else "strike"
    if strike_col not in ev.columns:
        raise ValueError("Events file must contain 'strike_K' or 'strike'.")
    ev = ev.copy()
...
def load_polymarket_long(long_path: Path, metadata_path: Path) -> pd.DataFrame:
    if not long_path.exists():
        raise FileNotFoundError(f"Polymarket long not found: {long_path}")
    ...
    if strike_col not in meta.columns:
        raise ValueError("Metadata must have strike_K or strike")
    if "expiry_date" not in meta.columns:
        raise ValueError("Metadata must have expiry_date (or parsed_date/date/expiry)")
    ...
    if "option_type" not in meta.columns:
        meta["option_type"] = "C"
    ...
    if "expiry_date_y" in merged.columns:
        merged = merged.drop(columns=["expiry_date_x"], errors="ignore").rename(columns={"expiry_date_y": "expiry_date"})
    merged = merged.loc[merged["expiry_date"].notna()].copy()
    logger.info("Loaded Polymarket long: %d rows (after metadata merge)", len(merged))
    return merged


def reduce_to_latest_per_strike(g: pd.DataFrame) -> pd.DataFrame:
    """Keep one row per strike: the row with the most recent timestamp_last."""
    idx = g.groupby("strike")["timestamp_last"].idxmax()
    return g.loc[idx].copy()


def apply_staleness_filter(g: pd.DataFrame, bucket_ts: pd.Timestamp) -> pd.DataFrame:
    """Keep only rows whose timestamp_last is within STALENESS_MINUTES of bucket_ts."""
    cutoff = bucket_ts - pd.Timedelta(minutes=STALENESS_MINUTES)
    return g.loc[g["timestamp_last"] >= cutoff].copy()


def fit_smile_for_snapshot_latest_only(
    g: pd.DataFrame,
    snapshot_spot: float,
    min_points: int = 3,
    min_range: float = 0.05,
) -> SmileFit:
    """Build smile from one row per strike (no averaging). Uses iv_norm from each row directly."""
    if g.empty:
        return SmileFit(False, "no_iv_data", np.nan, np.nan, 0, float("nan"), "", np.array([]), np.array([]))
    by_strike = g[["strike", "iv_norm"]].copy()
    by_strike = by_strike.drop_duplicates(subset=["strike"], keep="last")
    by_strike = by_strike.dropna(subset=["iv_norm"])
    if by_strike.empty:
        return SmileFit(False, "no_iv_data", np.nan, np.nan, 0, float("nan"), "", np.array([]), np.array([]))
    strike_num = pd.to_numeric(by_strike["strike"], errors="coerce")
    by_strike = by_strike.assign(m=strike_num / snapshot_spot).dropna(subset=["m", "iv_norm"])
    if by_strike.empty:
    ...
    iv_vals = by_strike["iv_norm"].to_numpy()
    ...
    total_contracts_sum = float(pd.to_numeric(g.get("contracts_sum", pd.Series(dtype=float)), errors="coerce").fillna(0.0).sum())
    ...
    interp_type = "pchip" if (HAS_PCHIP and n_unique >= 3) else "linear"
    return SmileFit(True, "", m_min, m_max, n_unique, total_contracts_sum, interp_type, m_unique, iv_unique)


def local_strike_gap_and_bracket(
    strike_k: float, m_nodes: np.ndarray, spot: float
) -> Tuple[float, float, float]:
    """Return (left_strike, right_strike, gap) for the bracket around strike_k. strike_nodes = m_nodes * spot."""
    strike_nodes = m_nodes * spot
    left_idx = np.searchsorted(strike_nodes, strike_k, side="right") - 1
    right_idx = np.searchsorted(strike_nodes, strike_k, side="left")
    if left_idx < 0:
        left_idx = 0
    if right_idx >= len(strike_nodes):
        right_idx = len(strike_nodes) - 1
    left_strike = float(strike_nodes[left_idx])
    right_strike = float(strike_nodes[right_idx])
    gap = right_strike - left_strike
    return left_strike, right_strike, gap
...
def build_smiles_and_evaluate(
    ...
    # Staleness / latest-per-strike diagnostics
    total_candidate_observations_before_dedup = 0
    total_strikes_after_latest_per_strike = 0
    total_retained_after_staleness = 0
    # Strike gap diagnostics
    in_range_gap_rule_fail_count = 0
    local_gaps_success: List[float] = []
    ...
    for bucket_ts in unique_buckets:
        ...
        for (expiry_date, opt_type), g in window_df.groupby(["expiry_date", "option_type"], sort=False):
            total_windows += 1
            total_candidate_observations_before_dedup += len(g)

            g_latest = reduce_to_latest_per_strike(g)
            total_strikes_after_latest_per_strike += len(g_latest)

            g_filtered = apply_staleness_filter(g_latest, bucket_ts)
            total_retained_after_staleness += len(g_filtered)

            spot_series = pd.to_numeric(g_filtered["index_price_last"], errors="coerce").dropna()
            ...
            fit = fit_smile_for_snapshot_latest_only(g_filtered, snapshot_spot=spot_median, min_points=MIN_POINTS)
            if fit.fit_success:
                fit_success_count += 1
            else:
            ...
            if fit is None:
                ...
                if not fit.fit_success:
                    ...
                    if fit.m_min <= m_event <= fit.m_max:
                        left_s, right_s, gap = local_strike_gap_and_bracket(
                            strike_k, fit.m_nodes, spot_median
                        )
                        if gap > MAX_STRIKE_GAP:
                            iv_hat = np.nan
                            out_of_range = True
                            out_of_range_count += 1
                            in_range_gap_rule_fail_count += 1
                        else:
    ...
    total_discarded_stale = total_strikes_after_latest_per_strike - total_retained_after_staleness
    staleness_diagnostics = {
        "total_candidate_strikes_before_deduplication": total_candidate_observations_before_dedup,
        "total_strikes_after_reducing_to_latest_trade_per_strike": total_strikes_after_latest_per_strike,
        "total_retained_strikes_after_staleness_filter": total_retained_after_staleness,
        "total_discarded_stale_strikes": total_discarded_stale,
        "avg_retained_strikes_per_snapshot": round(
            total_retained_after_staleness / total_windows, 4
        ) if total_windows else 0.0,
        "avg_discarded_stale_strikes_per_snapshot": round(
            total_discarded_stale / total_windows, 4
        ) if total_windows else 0.0,
    }

    avg_local_gap_success = float(np.mean(local_gaps_success)) if local_gaps_success else 0.0
    max_local_gap_success = float(np.max(local_gaps_success)) if local_gaps_success else 0.0
    strike_gap_diagnostics = {
        "in_range_gap_rule_fail_count": in_range_gap_rule_fail_count,
        "avg_local_gap_successful_interpolation": round(avg_local_gap_success, 4),
        "max_local_gap_successful_interpolation": round(max_local_gap_success, 4),
    }
    ...
    if not snapshot_stats.empty:
        ...
        if n_snapshot_rows == n_unique_ts and n_snapshot_rows > 0:
            ...
            validation_warning = "Snapshot stats has exactly one row per timestamp; check grouping."
    ...
    if not event_iv.empty and "event_date" in event_iv.columns and "expiry_utc" in event_iv.columns:
        ed = pd.to_datetime(event_iv["event_date"])
        ex = pd.to_datetime(event_iv["expiry_utc"])
        if (ex > ed + pd.Timedelta(days=1)).any():
            validation_expiry_warning = "Some rows have expiry_utc later than event_date + 1 day."
    ...
    summary: Dict[str, Any] = {
        "latest_trade_per_strike_rule": True,
        ...
        "strike_gap_diagnostics": strike_gap_diagnostics,
    }
...
def make_plots(snapshot_stats: pd.DataFrame, aux: Dict[str, Any], out_dir: Path) -> None:
    ...
    try:
        ...
        ax.set_ylabel("count")
        ax.set_title("Distribution of unique strikes (3h + 90min staleness + latest/trade, min3, gap10k)")
        fig.tight_layout()
        fig.savefig(plots_dir / "n_unique_strikes_hist.png", dpi=150)
        plt.close(fig)
    except Exception as e:
        logger.warning("Failed n_unique_strikes_hist: %s", e)
    try:
        ...
        ax.set_xlabel("date")
        ax.set_title("Smile fit success rate by day (3h + 90min + min3 + gap10k)")
        fig.autofmt_xdate()
        fig.tight_layout()
        fig.savefig(plots_dir / "fit_success_rate_by_day.png", dpi=150)
        plt.close(fig)
    except Exception as e:
        logger.warning("Failed fit_success_rate_by_day: %s", e)
    try:
        ...
        if example_smiles:
            ...
            for (bucket_ts, expiry_date, opt_type, m_nodes, iv_nodes) in example_smiles:
                ...
                ax.plot(m_grid, iv_grid, label=f"{expiry_date} {opt_type} {bucket_ts}")
                ax.scatter(m_nodes, iv_nodes, s=10, alpha=0.7)
            ax.set_xlabel("moneyness (strike / spot)")
            ax.set_ylabel("IV (decimal)")
            ax.set_title("Example smiles (3h + 90min + min3 + gap10k)")
            ax.legend(fontsize=7)
            fig.tight_layout()
            fig.savefig(plots_dir / "example_smiles.png", dpi=150)
            plt.close(fig)
    except Exception as e:
        logger.warning("Failed example_smiles: %s", e)


def write_timestamp_coverage(event_iv_path: Path, output_path: Path) -> None:
    df = pd.read_parquet(event_iv_path, columns=["bucket_5m_utc", "iv_hat"])
    ...
    timestamps_with_iv = df.loc[usable_iv, "bucket_5m_utc"].nunique()
    timestamps_without_iv = total_unique_timestamps - timestamps_with_iv
    pct_timestamps_with_iv = (100.0 * timestamps_with_iv / total_unique_timestamps) if total_unique_timestamps else 0.0
    per_ts = df.groupby("bucket_5m_utc")["iv_hat"].apply(lambda s: (s.notna() & np.isfinite(s)).sum())
    avg_usable = float(per_ts.mean()) if len(per_ts) else 0.0
    median_usable = float(per_ts.median()) if len(per_ts) else 0.0
    max_usable = int(per_ts.max()) if len(per_ts) else 0
    ...
    for d in sorted(df["date"].unique()):
        ...
        daily_coverage.append({
            "date": str(d), "timestamps": int(ts_tot), "timestamps_with_iv": int(ts_iv), "pct": round(pct, 4)
        })
    out = {
        ...
        "iv_density_per_timestamp": {
            "avg_usable_iv_per_timestamp": round(avg_usable, 4),
            "median_usable_iv_per_timestamp": round(median_usable, 4),
            "max_usable_iv_per_timestamp": max_usable,
        },
...
def main() -> None:
    ...
    for name in [
        "options_smile_snapshot_stats.parquet", "options_smile_event_iv.parquet",
        "options_smile_report.json", "options_iv_timestamp_coverage.json",
    ]:
        p = out_dir / name
        if p.exists():
            p.unlink()
    ...
    logger.info("Loading Polymarket events and long")
    events_df = load_events(events_path)
    polymarket_long = load_polymarket_long(polymarket_long_path, events_path)

    logger.info("Building smiles (3h + 90min staleness + latest/trade + min3 + gap10k)")
    snapshot_stats, event_iv, aux = build_smiles_and_evaluate(
        options_df, events_df, polymarket_long=polymarket_long
    )

    snapshot_path = out_dir / "options_smile_snapshot_stats.parquet"
    event_iv_path = out_dir / "options_smile_event_iv.parquet"
    if not snapshot_stats.empty:
    ...
    if not summary.get("row_count_match_ok"):
        summary["sanity_check_warning"] = "Dataset 2 row count does not match Polymarket long."
    report = {
    ...
    report_path = out_dir / "options_smile_report.json"
    report_path.write_text(json.dumps(report, indent=2), encoding="utf-8")
    logger.info("Wrote %s", report_path)

    make_plots(snapshot_stats, aux, out_dir)
    if event_iv_path.exists():
        write_timestamp_coverage(event_iv_path, out_dir / "options_iv_timestamp_coverage.json")
