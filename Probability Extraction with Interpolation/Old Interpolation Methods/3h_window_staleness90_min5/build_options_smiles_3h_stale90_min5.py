"""Build options IV smiles: 3h window + per-strike 90min staleness, min 5 strikes.

Methodology variant 2: outputs go to this folder only.
- Rolling lookback: 3 hours [t - 3h, t].
- Per-strike staleness: keep strike only if (t - last_trade_time) <= 90 minutes.
- Minimum unique strikes >= 5.
- Same interpolation (PCHIP/linear), no extrapolation, same Polymarket expiry rule.
"""
...
def _root() -> Path:
    # Script lives in analysis/3h_window_staleness90_min5/
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build options smiles: 3h window, 90min staleness, min 5 strikes.")
    parser.add_argument(
        "--options_path",
        type=str,
...
def load_options(path: Path) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Options file missing required columns (staleness needs timestamp_last): {missing}")
...
def load_events(path: Path) -> pd.DataFrame:
    ...
    if "expiry_date" not in ev.columns:
        raise ValueError(f"Events file must contain 'expiry_date'.")
    strike_col = "strike_K" if "strike_K" in ev.columns else "strike"
    if strike_col not in ev.columns:
        raise ValueError(f"Events file must contain 'strike_K' or 'strike'.")
    ev = ev.copy()
...
def load_polymarket_long(long_path: Path, metadata_path: Path) -> pd.DataFrame:
    ...
    if missing.any():
        n = int(missing.sum())
        logger.warning("Dropping %d long rows with no metadata.", n)
...
def fit_smile_for_snapshot(
    ...
    if "iv_strike" not in by_strike.columns:
        by_strike = by_strike.rename(columns={by_strike.columns[-1]: "iv_strike"})
    by_strike = by_strike.dropna(subset=["iv_strike"])
    if by_strike.empty:
    ...
    by_strike = by_strike.assign(m=strike_num / snapshot_spot).dropna(subset=["m", "iv_strike"])
    if by_strike.empty:
...
def apply_staleness_filter(g: pd.DataFrame, bucket_ts: pd.Timestamp) -> pd.DataFrame:
    """Keep only strikes whose last trade time is within STALENESS_MINUTES of bucket_ts."""
    # Per-strike most recent trade time
    strike_last_time = g.groupby("strike")["timestamp_last"].max()
    cutoff = bucket_ts - pd.Timedelta(minutes=STALENESS_MINUTES)
    fresh_strikes = strike_last_time[strike_last_time >= cutoff].index
    return g.loc[g["strike"].isin(fresh_strikes)].copy()
...
def build_smiles_and_evaluate(
    ...
    total_candidate_strikes_before_staleness = 0
    total_retained_strikes_after_staleness = 0
    ...
    for bucket_ts in unique_buckets:
        ...
        for (expiry_date, opt_type), g in window_df.groupby(["expiry_date", "option_type"], sort=False):
            total_windows += 1
            n_candidate_strikes = g["strike"].nunique()
            total_candidate_strikes_before_staleness += n_candidate_strikes

            g_filtered = apply_staleness_filter(g, bucket_ts)
            n_retained_strikes = g_filtered["strike"].nunique()
            total_retained_strikes_after_staleness += n_retained_strikes
            ...
            fit = fit_smile_for_snapshot(g_filtered, snapshot_spot=spot_median)
            if fit.fit_success:
                fit_success_count += 1
            else:
    ...
    # Dataset 2: one row per Polymarket long row
    ...
    if not event_iv.empty:
        ...
        if duplicate_event_keys > 0:
            logger.warning("Dataset 2 has %d duplicate event keys; dropping to first.", duplicate_event_keys)
    ...
    if row_count_polymarket_long and row_count_event_iv_output != row_count_polymarket_long:
        logger.warning("Dataset 2 row count mismatch: long=%d, event_iv=%d", row_count_polymarket_long, row_count_event_iv_output)
    ...
    if not event_iv.empty and "event_date" in event_iv.columns and "expiry_utc" in event_iv.columns:
        ...
        at_or_before_08 = (hour < DAILY_EXPIRY_CUTOFF_HOUR_UTC) | ((hour == DAILY_EXPIRY_CUTOFF_HOUR_UTC) & (minute == 0))
        ...
        invalid_expiry_assignments = int(np.sum((same_day_required & ~used_same) | (next_day_required & ~used_next)))
        if invalid_expiry_assignments > 0:
            logger.warning("Dataset 2: %d rows have invalid expiry assignment.", invalid_expiry_assignments)
    ...
    # Staleness diagnostics
    total_discarded_strikes = total_candidate_strikes_before_staleness - total_retained_strikes_after_staleness
    staleness_diagnostics = {
        "total_candidate_strikes_before_staleness": total_candidate_strikes_before_staleness,
        "total_retained_strikes_after_staleness": total_retained_strikes_after_staleness,
        "total_discarded_stale_strikes": total_discarded_strikes,
        "avg_retained_strikes_per_snapshot": round(total_retained_strikes_after_staleness / total_windows, 4) if total_windows else 0.0,
        "avg_discarded_stale_strikes_per_snapshot": round(total_discarded_strikes / total_windows, 4) if total_windows else 0.0,
    }
...
def make_plots(snapshot_stats: pd.DataFrame, aux: Dict[str, Any], out_dir: Path) -> None:
    ...
    try:
        ...
        ax.set_ylabel("count")
        ax.set_title("Distribution of unique strikes per window (3h + 90min staleness)")
        fig.tight_layout()
    ...
    try:
        ...
        ax.set_xlabel("date")
        ax.set_title("Smile fit success rate by day (3h + 90min staleness)")
        fig.autofmt_xdate()
        fig.tight_layout()
    ...
    try:
        ...
        if example_smiles:
            ...
            ax.set_title("Example per-window smiles (3h + 90min staleness)")
...
def write_timestamp_coverage(event_iv_path: Path, output_path: Path) -> None:
    """Compute options_iv_timestamp_coverage.json from event_iv parquet."""
    ...
    for d in sorted(df["date"].unique()):
        ...
        daily_coverage.append({"date": str(d), "timestamps": int(ts_tot), "timestamps_with_iv": int(ts_iv), "pct": round(pct, 4)})
    out = {
    ...
    print("avg_usable_iv_per_timestamp:", avg_usable)
...
def main() -> None:
    ...
    # Clean only this methodology folder outputs (not baseline)
    for name in ["options_smile_snapshot_stats.parquet", "options_smile_event_iv.parquet", "options_smile_report.json", "options_iv_timestamp_coverage.json"]:
        p = out_dir / name
        if p.exists():
            p.unlink()
    ...
    logger.info("Loading Polymarket 5m long from %s", polymarket_long_path)
    polymarket_long = load_polymarket_long(polymarket_long_path, events_path)

    logger.info("Building smiles (3h window, 90min staleness, min 5 strikes)")
    ...
    coverage_path = out_dir / "options_iv_timestamp_coverage.json"
    if event_iv_path.exists():
        write_timestamp_coverage(event_iv_path, coverage_path)
