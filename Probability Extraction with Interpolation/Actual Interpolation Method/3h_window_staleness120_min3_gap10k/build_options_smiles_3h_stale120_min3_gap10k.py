"""Build options IV smiles: 3h window + 120min staleness + min 3 strikes + max gap 10k.

Methodology variant: outputs go to this folder only.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build options smiles: 3h window, 120min staleness, min 3 strikes, max gap 10k."
    )
    parser.add_argument(
        "--options_path",
        type=str,
...
def make_plots(snapshot_stats: pd.DataFrame, aux: Dict[str, Any], out_dir: Path) -> None:
    ...
    try:
        ...
        ax.set_ylabel("count")
        ax.set_title("Distribution of unique strikes (3h + 120min staleness + latest/trade, min3, gap10k)")
        fig.tight_layout()
    ...
    try:
        ...
        ax.set_xlabel("date")
        ax.set_title("Smile fit success rate by day (3h + 120min + min3 + gap10k)")
        fig.autofmt_xdate()
        fig.tight_layout()
    ...
    try:
        ...
        if example_smiles:
            ...
            ax.set_title("Example smiles (3h + 120min + min3 + gap10k)")
...
def main() -> None:
    ...
    logger.info("Building smiles (3h + 120min staleness + latest/trade + min3 + gap10k)")
