"""News Event Study: Polymarket vs option-implied probabilities around news.

Goal
-----
Test whether the probability spread between Polymarket probabilities and
option-implied probabilities becomes larger around major news shocks.
...
All outputs are written ONLY under:
  Econometric Analysis/News Event Study/
...
OUTPUT_ROOT = CODE_ROOT / "Econometric Analysis" / "News Event Study"
TABLES_DIR = OUTPUT_ROOT / "tables"
FIGURES_DIR = OUTPUT_ROOT / "figures"
DIAGNOSTICS_DIR = OUTPUT_ROOT / "diagnostics"


EVENT_WINDOW_HOURS = 12.0
...
def load_probability_data() -> pd.DataFrame:
    ...
    # Use bucket_5m_utc as the timestamp for the event study logic.
    df = df.copy()
...
def parse_timestamps(prob_df: pd.DataFrame, events_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
...
def assign_event_windows(
    ...
    Rules:
      - If within any pre_event window [t-12h, t), label pre_event.
      - If within any post_event window [t, t+12h], label post_event.
      - If an observation falls into multiple windows, assign to the event with
        minimum absolute time distance.
    ...
    # Precompute timestamp array
    ts = df["timestamp"]

    # For each event, update assignment where this event is closer than current
    ...
    for _, ev in events_df.iterrows():
        ...
        # Compute time difference in hours (prob - event)
        delta = (ts - ev_time).dt.total_seconds() / 3600.0
        abs_delta = delta.abs()

        # Within +/- EVENT_WINDOW_HOURS
        in_window = abs_delta <= EVENT_WINDOW_HOURS
        if not in_window.any():
            continue

        # For those rows where this event is closer than previous assignment
        better = in_window & (abs_delta < best_abs_delta)
        if not better.any():
            continue

        # Update best event assignment
    ...
    # Set regimes based on event_time_hours
...
def run_tests(df: pd.DataFrame) -> pd.DataFrame:
    ...
    res.append(
        _ttest_wrapper(
            ...
            crypto_post,
            macro_post,
            test_name="D_crypto_post_vs_macro_post",
...
def compute_event_level_summary(
    ...
    """Compute pre/post means by event_id."""
    window_df = df[df["regime"].isin(["pre_event", "post_event"])].copy()
    if window_df.empty:
        return pd.DataFrame()
...
def plot_event_time_abs_spread(df: pd.DataFrame) -> None:
    """Figure 1: Average abs_spread by event time (1-hour bins)."""
    window_df = df[df["regime"].isin(["pre_event", "post_event"])].copy()
    window_df = window_df.dropna(subset=["event_time_hours", "abs_spread"])
    if window_df.empty:
        logger.warning("No event-window observations for Figure 1.")
        return
    ...
    plt.title("Average absolute probability spread by event time")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    out_path = FIGURES_DIR / "event_time_abs_spread.png"
    plt.savefig(out_path, dpi=150)
    plt.close()
    logger.info("Saved Figure 1 to %s", out_path)


def plot_abs_spread_by_regime(df: pd.DataFrame) -> None:
    """Figure 2: Boxplot of abs_spread by regime."""
    regimes = ["normal", "pre_event", "post_event"]
    data = [df.loc[df["regime"] == r, "abs_spread"].dropna() for r in regimes]
    if all(len(x) == 0 for x in data):
        logger.warning("No data for Figure 2 boxplot.")
        return

    plt.figure(figsize=(6, 4.5))
    plt.boxplot(data, labels=regimes, showfliers=False)
    plt.ylabel("|probability spread|")
    plt.title("Distribution of absolute probability spread by regime")
    plt.tight_layout()
    out_path = FIGURES_DIR / "abs_spread_by_regime_boxplot.png"
    plt.savefig(out_path, dpi=150)
    plt.close()
    logger.info("Saved Figure 2 to %s", out_path)


def plot_abs_spread_by_category(df: pd.DataFrame) -> None:
    """Figure 3: Bar chart of mean abs_spread by category and regime."""
    window_df = df[df["regime"].isin(["pre_event", "post_event"])].copy()
    window_df = window_df.dropna(subset=["event_category"])
    if window_df.empty:
        logger.warning("No event-window data for Figure 3.")
        return
    ...
    plt.ylabel("Mean |probability spread|")
    plt.title("Mean absolute probability spread by category and regime")
    plt.legend()
    plt.tight_layout()
    out_path = FIGURES_DIR / "abs_spread_by_category.png"
    plt.savefig(out_path, dpi=150)
    plt.close()
    logger.info("Saved Figure 3 to %s", out_path)
...
def write_summary_text(
    ...
    """Write high-level summary for thesis use."""
    out_path = TABLES_DIR / "news_event_study_summary.txt"
    ...
    lines: List[str] = []
    lines.append("News Event Study – Summary")
    lines.append("===================================")
    lines.append("")
    ...
    lines.append("")
    lines.append("Key t-tests on abs_spread (difference in means, Welch t-tests):")
    ...
    lines.append(f"  - {TABLES_DIR / 'news_event_regime_summary.csv'}")
    lines.append(f"  - {TABLES_DIR / 'news_event_category_summary.csv'}")
    lines.append(f"  - {TABLES_DIR / 'news_event_tests.csv'}")
    lines.append(f"  - {TABLES_DIR / 'news_event_level_summary.csv'}")
    lines.append(f"  - {FIGURES_DIR / 'event_time_abs_spread.png'}")
    lines.append(f"  - {FIGURES_DIR / 'abs_spread_by_regime_boxplot.png'}")
    lines.append(f"  - {FIGURES_DIR / 'abs_spread_by_category.png'}")

    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    logger.info("Wrote summary text to %s", out_path)
...
def write_diagnostics(
    ...
    """Write diagnostics file and return its content."""
    out_path = DIAGNOSTICS_DIR / "news_event_study_diagnostics.txt"
    ...
    lines: List[str] = []
    lines.append("News Event Study – Diagnostics")
    lines.append("===================================")
    ...
    lines.append(f"  - {FIGURES_DIR / 'abs_spread_by_regime_boxplot.png'}")
    lines.append(f"  - {FIGURES_DIR / 'abs_spread_by_category.png'}")
    lines.append(f"  - {TABLES_DIR / 'news_event_study_summary.txt'}")
...
def main() -> None:
    ...
    # Assign regimes and event-time variable
    df = assign_event_windows(prob_df, events_df)

    # Descriptive tables
    regime_summary = compute_regime_summary(df)
    regime_summary.to_csv(
        TABLES_DIR / "news_event_regime_summary.csv", index=False
    )

    category_summary = compute_category_summary(df)
    category_summary.to_csv(
        TABLES_DIR / "news_event_category_summary.csv", index=False
    )

    # Statistical tests
    tests_df = run_tests(df)
    tests_df.to_csv(TABLES_DIR / "news_event_tests.csv", index=False)

    # Event-level summary
    event_level_summary = compute_event_level_summary(df, events_df)
    event_level_summary.to_csv(
        TABLES_DIR / "news_event_level_summary.csv", index=False
    )
